 ...
 * @author ahmed
 */
public class SalesBillsPanel extends JPanel implements ActionListener, ItemListener, CaretListener, ChangeListener {
    ...
    String text;
    int totalfromNumber, totaltoNumber, profitfromNumber, profittoNumber;
    int x = 0;
    int selectedrow;
    Date fromdate, todate;
    private JTable table;
    String[] columnNames = {"Name", "UnitPrice", "UnitNumber", "Total", "discount ", "Net"};
    String[] items = {"------", "ALL", "BILL-CODE", "DATE", "TOTAL", "PROFIT"};
    String[] header = {"BILL-CODE", "TOTAL", "PROFIT", "DATE"};
    ...
    public SalesBillsPanel() throws SQLException {
        //-----------------------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            public void mouseClicked(MouseEvent e) {
                if (e.getClickCount() == 2) {
                    ...
                    try {
                        ...
                        Logger.getLogger(SalesBillsPanel.class.getName()).log(Level.SEVERE, null, ex);
                    }
                }
            }
        });
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == search) {
            ...
            if (x == 1) {
                ...
                List<SalesBillsBean> l = getWay.listAllSales();
                if (l != null && l.size() > 0) {
                    int i = 0;
                    for (i = 0; i < l.size(); i++) {
                        defTableModel.addRow(new Object[]{l.get(i).getBillcode(), l.get(i).getTotalnet(), l.get(i).getTotalprofits(), l.get(i).getBilldate()});
                    }
                }
            }
            //----------------------------------------------------------------select bybill code----------------------------------------------------------------------------
            if (x == 2) {
                defTableModel.setNumRows(0);
                List<SalesBillsBean> l = getWay.findByCodeSales(Integer.parseInt(text));
                if (l != null && l.size() > 0) {
                    defTableModel.addRow(new Object[]{l.get(0).getBillcode(), l.get(0).getTotalnet(),
                        l.get(0).getTotalprofits(), l.get(0).getBilldate()});
                }
            }
            ...
            if (x == 3) {
                ...
                defTableModel.setNumRows(0);
                List<SalesBillsBean> l = getWay.findByTotalSales(totalfromNumber, totaltoNumber);
                if (l != null && l.size() > 0) {
                    int i = 0;
                    for (i = 0; i < l.size(); i++) {
                        defTableModel.addRow(new Object[]{l.get(i).getBillcode(), l.get(i).getTotalnet(), l.get(i).getTotalprofits(), l.get(i).getBilldate()});
            ...
            if (x == 4) {
                try {
                    ...
                    List<SalesBillsBean> l = getWay.findByDateSales(fromdate, todate);
                    if (l != null && l.size() > 0) {
                        int i = 0;
                        for (i = 0; i < l.size(); i++) {
                            defTableModel.addRow(new Object[]{l.get(i).getBillcode(), l.get(i).getTotalnet(), l.get(i).getTotalprofits(), l.get(i).getBilldate()});
...
//                       
            if (x == 6) {
                defTableModel.setNumRows(0);
                List<SalesBillsBean> l = getWay.findByProfitSales(profitfromNumber, profittoNumber);
                if (l != null && l.size() > 0) {
                    int i = 0;
                    for (i = 0; i < l.size(); i++) {
                        defTableModel.addRow(new Object[]{l.get(i).getBillcode(), l.get(i).getTotalnet(), l.get(i).getTotalprofits(), l.get(i).getBilldate()});
    ...
    public void itemStateChanged(ItemEvent ex) {
        if (ex.getSource() == cb) {
            ...
            if (cb.getSelectedItem().equals("PROFIT")) {
                profitSelectedMethod();
                x = 6;
            }
    ...
    private JPanel profitSelectedMethod() {
        namePanel = new JPanel();
        namePanel.setBackground(Color.cyan);
        JLabel from = new JLabel("FROM");
        JLabel TO = new JLabel("TO");
        SpinnerModel model = new SpinnerNumberModel(50, 1, 1000, 10);
        SpinnerModel models = new SpinnerNumberModel(50, 1, 1000, 10);
        profitfromSpiner = new JSpinner(model);
        profitfromSpiner.addChangeListener(this);
        profitfromSpiner.setPreferredSize(new Dimension(150, 50));
        profittoSpiner = new JSpinner(models);
        profittoSpiner.addChangeListener(this);
        profittoSpiner.setPreferredSize(new Dimension(150, 50));
        namePanel.add(from);
        namePanel.add(profitfromSpiner);
        namePanel.add(TO);
        namePanel.add(profittoSpiner);
        TitledBorder border1 = new TitledBorder("BILL PROFIT ");
        border1.setTitleJustification(TitledBorder.LEFT);
        border1.setTitlePosition(TitledBorder.TOP);
        westsearchPanel.setBorder(border1);
        westsearchPanel.removeAll();
    ...
    public void stateChanged(ChangeEvent e) {
        ...
        if (e.getSource() == profitfromSpiner) {
            profitfromNumber = (int) profitfromSpiner.getValue();

        }
        if (e.getSource() == profittoSpiner) {
            profittoNumber = (int) profittoSpiner.getValue();
    ...
    public JDialog viewdialog() throws SQLException {
        ...
        SalesBillsPanel.defTableModel2.setRowCount(0);
        int bill_code_selected = (int) exampleJTable.getValueAt(selectedrow, 0);
        defTableModel2.setNumRows(0);
        List<SalesBillsBean> l = getWay.findByCodeSales(bill_code_selected);
        if (l.get(0).getDrug1() != null) {
            defTableModel2.addRow(new Object[]{l.get(0).getDrug1(),
                l.get(0).getUnitprice1(), l.get(0).getQuantity1(),
                l.get(0).getTotal1(), l.get(0).getDiscount1(), l.get(0).getNet1()});

        }
        if (l.get(0).getDrug2() != null) {
            defTableModel2.addRow(new Object[]{l.get(0).getDrug2(),
                l.get(0).getUnitprice2(), l.get(0).getQuantity2(),
                l.get(0).getTotal2(), l.get(0).getDiscount2(), l.get(0).getNet2()});
